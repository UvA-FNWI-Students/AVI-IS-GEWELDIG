...
function setupButtonEvasion() {
    const eensButton = document.getElementById('eens') as HTMLButtonElement;
    const oneensButton = document.getElementById('oneens') as HTMLButtonElement;

    if (!eensButton || !oneensButton) return;
    ...
    function evadeButton(event: MouseEvent) {
        if (isEvading) return;
        isEvading = true;
